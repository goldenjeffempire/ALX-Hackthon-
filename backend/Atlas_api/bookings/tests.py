...
class BookingModelTest(TestCase):
    def setUp(self):
        ...
        self.workspace = Workspace.objects.create(
            name="Desk 1", space_type="desk", capacity=1
        )
    ...
    def test_valid_booking_creation(self):
        ...
        self.assertEqual(
            str(booking),
            f"{self.user.username} - {self.workspace.name} ({booking.start_time} to {booking.end_time})",
        )
...
class NotificationModelTest(TestCase):
    def setUp(self):
        self.user = CustomUser.objects.create_user(
            username="notifier", password="testpass"
        )
        self.workspace = Workspace.objects.create(
            name="Desk 3", space_type="desk", capacity=1
        )
...
class WorkspaceUsageReportModelTest(TestCase):
    ...
    def test_report_creation(self):
        ...
        self.assertEqual(
            str(self.report),
            f"Usage Report {self.report.start_date} to {self.report.end_date}",
        )
